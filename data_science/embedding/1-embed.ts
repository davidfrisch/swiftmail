...
const chunklengths = [25, 50, 100, 250, 500, 1000];
const chunkoverlaps = [0, 10, 25, 50, 100];
    ...
    if (overlap < length) {
      ...
      for await (const file of glob.scan("scripts/")) {
        ...
        for await (const chunk of chunks) {
          const embed = (await ollama.embeddings({ model: embeddedModel, prompt: chunk })).embedding
